...
abstract class TweetSet {
  ...
  def mostRetweetedHelper(t: Tweet): Tweet
  /**
...
class Empty extends TweetSet {
  ...
  def mostRetweetedHelper(t: Tweet) = t
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedHelper(t: Tweet): Tweet = {
    val max = if (elem.retweets > t.retweets) elem else t
    val temp = left.mostRetweetedHelper(max)
    right.mostRetweetedHelper(temp)
  }

  def mostRetweeted = mostRetweetedHelper(new Tweet("","", -1))
