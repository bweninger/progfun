...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = b.legalNeighbors.map((neighbor : (Block, Move)) => (neighbor._1, neighbor._2 :: history)).toStream
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.filter {
      case (blk, _) => !(explored.contains(blk))
    }
  }
