...
case class ProductCode(productCode : Option[String], discountCode : Option[String], description : Option[String])

// Definition of the ProductCod table
object ProductCode extends Table[ProductCode]("PRODUCT_CODE") {

  //def id = column[Int]("ID", O.PrimaryKey, O AutoInc) // This is the primary key column
  
  
  def productCode = column[Option[String]]("PRODUCT_CODE", O.PrimaryKey) 
  
  
  def discountCode = column[Option[String]]("DISCOUNT_CODE") 
  // def discountCodeEntity = foreignKey("SYS_FK_10110", discountCode, DiscountCode)(_.id)
  
  def description = column[Option[String]]("DESCRIPTION") 
  
    
  
  def * = productCode ~ discountCode ~ description <> (ProductCode.apply _, ProductCode.unapply _)
  //def autoInc = id.? ~ productCode.? ~ discountCode ~ description <> (ProductCod.apply _, ProductCod.unapply _) returning id 
  ...
  def findByPK(pk: String) =
    for (entity <- ProductCode if entity.productCode === pk) yield entity
  
    
  /**
   * Construct the Map[String,String] needed to fill a select options set.
   */
  def options = this.findAll().map(x => x.productCode -> productCode)
  
  val mirror = ru.runtimeMirror(getClass.getClassLoader)
  
  def list(page: Int = 0, pageSize: Int = 10, orderBy: Int = 1, order: String, asc: Boolean = false, filter: String = "%") = {
    val members = ru.typeOf[ProductCode].members.filter(m => m.isTerm && !m.isMethod).toList
    ...
    val methodFields = sortField match {
      ...
      case "productCode" => ru.typeOf[ProductCode.type].declaration(ru.newTermName("productCode")).asMethod
